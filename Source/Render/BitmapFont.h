...
static constexpr int k_font_width  = 5;
static constexpr int k_font_height = 7;
static constexpr int k_font_first  = 32;
static constexpr int k_font_last   = 126;
static constexpr int k_font_count  = k_font_last - k_font_first + 1;

// 5x7 bitmap font, ASCII 32-126.
// 7 rows per glyph (top to bottom), bits 4..0 = pixels left to right.
static constexpr uint8_t k_font_5x7[k_font_count][k_font_height] = {
...
// Returns pointer to 7-byte glyph row data, or nullptr for out-of-range.
inline const uint8_t* font_glyph(char c) {
    int idx = static_cast<int>(static_cast<unsigned char>(c)) - k_font_first;
    if (idx < 0 || idx >= k_font_count) return nullptr;
    return k_font_5x7[idx];
}
